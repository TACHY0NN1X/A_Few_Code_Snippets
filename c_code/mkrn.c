/* Turing Pattern, Reaction diffusion Image Generation
 * Using C, Based on code example from the Coding Train
 *
...
int main(){
   ...
   int r = width/3;
   for (float theta=0.0; theta<2*M_PI;theta+=0.0225){
      int x = (int)(r*cos(theta)) + w_2;
      int y = (int)(r*sin(theta)) + w_2;
      grid[idx(x,y)].b = 1;
      //grid[idx(x-10,y-10)].b = 1;
      //grid[idx(x-20,y-20)].b = 1;
   }
   
   puts("Computing Image \t-->");
   
   int max_iterations = 4900;
   int iterations = 0;
   float percent = 0.0;
